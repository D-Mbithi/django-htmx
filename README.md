...
1. Buat direktori untuk proyek yang akan kamu buat (contoh: `core`), lalu
...
5. Buat proyek Django baru dengan templat ini.
   ...
   ```shell
   django-admin startproject --template="https://codeload.github.com/laymonage/django-template-heroku/zip/template" --extension="py,yml,md" --name="Procfile" core .
   ```

   > Catatan: ubah `core` dengan nama proyek yang kamu inginkan.
...
3. Atur konfigurasi *code coverage* di GitLab.
   ...
   *Code coverage* merupakan salah satu metrik yang berguna untuk mengukur
   ...
   ```shell
   coverage run --include="./*" --omit="venv/*,manage.py,core/*" manage.py test
   ```
